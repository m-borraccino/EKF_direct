...
xlabel('yEast [m]')
ylabel('xNorth [m]')
axis([p_xmin-5 p_xmax+5 p_ymin-5 p_ymax+45])
...
xlabel('yEast [m]')
ylabel('xNorth [m]')
axis([p_xmin-5 p_xmax+5 p_ymin-5 p_ymax+55])
GPS_data = plot(out.GPS_NED.data(tt_GPS,2),out.GPS_NED.data(tt_GPS,1),'yellow*'); 
plot(out.GT_pos.data(tt1,2),out.GT_pos.data(tt1,1),'black-');
real_tr = plot(out.GT_pos.data(tt2,2),out.GT_pos.data(tt2,1),'black-');
...
grid on
xlabel('Time [s/100]')
xlim([0 out.SimulationMetadata.ModelInfo.StopTime*100])
